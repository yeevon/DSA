...
- [x] Each chapter `ch_N` for N ∈ {1, 2, 3, 4, 5, 6} has a Step-1
...
- [x] Each chapter `ch_N` for N ∈ {1, 2, 3, 4, 5, 6} has a Step-2
      gap report at `design_docs/chapter_reviews/ch_<N>_gaps.md`.
      *Verify:* same loop with `_gaps.md` suffix.
- [x] Each chapter `ch_N` for N ∈ {1, 2, 3, 4, 5, 6} has a Step-3
...
- [x] All 12 chapters' `lectures.tex` and `notes.tex` (24 files
...
- [x] No `lectures.pdf` exceeds 40 pages without an explicit
      acknowledgement in the build-sweep doc (per
      `feedback_chapter_review_autonomy.md`'s autonomy ceiling).

### Cross-reference correctness

- [x] No `chapters/ch_*/lectures.tex` references a non-existent
...
- [x] AVL / red-black trees are referenced as **ch.~9** (not
...
- [x] Optional chapters (ch_7, ch_9–ch_13) are explicitly enumerated
