...
**Status:** ✅ **PASS** (genuinely, all-issues-actioned) as of 2026-04-25 close-out — second pass, after user pushback on the first close-out which had left 2 MEDIUMs and 7 LOWs OPEN. Final dispositions:

- **2 HIGH RESOLVED:** DR-01 (bullet 10 stale numbers — 40 pages / +756 KB / +17.5% / 37 carriers / 22 cases / 34 asserts post-second-pass); DR-04 (T8-promised harness-extension entry — `nice_to_have.md §UX-3` appended).
- **3 MEDIUM RESOLVED:** DR-02 (architecture.md §1.6 — collection-landing pages paragraph + RHS-TOC top-level filter paragraph appended); DR-03 (architecture.md gains new `### §1.7 Verification gates` subsection covering both harnesses + the selenium pin + the interactive-mode boundary); DR-05 (CompletionIndicator ~432 KB residual — `nice_to_have.md §UX-4` appended with M5-coupled trigger).
- **8 LOW addressed:** DR-06 (bullet 5 b29d409 mention folded in); DR-07 (chrome.css unused tokens RESOLVED-WITH-RATIONALE — accept-as-is per auditor option (a)); **DR-08 RESOLVED with real impact** — the breadcrumb-height invariant test landed AND caught actual drift (token said 52px, rendered was 44.91px after T8 sticky-rule relocation; token re-measured to 46px, rails now sit 7px tighter to the breadcrumb); DR-09 (mobile coverage — 2 new test cases at 375×812 covering drawer-trigger visibility + RHS TOC closed-by-default); DR-10 (no-sandbox argparse — DEFERRED to T9-ISS-03 ergonomics sweep, explicitly logged); DR-11 + DR-12 (HomeLayout duplication + 4-page scoped-CSS RESOLVED-WITH-RATIONALE per auditor option (a)); DR-13 (subsumed by DR-09).
- **Functional-test harness post-pass:** **22 test cases / 34 assertions** all PASS in 10.6s. Mobile breakpoints (375) now in the assertion suite. Breadcrumb-height invariant locks the sticky-rail offset against future drift.

M-UX milestone genuinely closes here; no T10 needed; no item left "DEFERRED" without an explicit destination (DR-10 is the lone deferral and is logged at T9-ISS-03, not handwaved).
...
| M-UX-DR-02        | 🟡 MEDIUM | ✅ RESOLVED 2026-04-25 (orchestrator at close-out) | architecture.md §1.6 amended with collection-landing-pages paragraph + RHS-TOC top-level filter paragraph |
| M-UX-DR-03        | 🟡 MEDIUM | ✅ RESOLVED 2026-04-25 (orchestrator at close-out) | architecture.md gains new `### §1.7 Verification gates` subsection covering smoke-screenshots.py + functional-tests.py + _selenium_helpers.py + selenium pin + interactive-mode boundary |
...
| M-UX-DR-06        | 🟢 LOW   | ✅ RESOLVED 2026-04-25 (folded into DR-01 fix) | milestone README bullet 5 amended with the `^\d+\.\d+\s` filter clarification + b29d409 attribution |
| M-UX-DR-07        | 🟢 LOW   | ✅ RESOLVED-WITH-RATIONALE 2026-04-25 — option (a) per auditor recommendation | tokens left as-is; intent-as-documentation idiom preserved; future M5 / dark-mode sweep may consume them. Re-flag if a future audit finds them confusing rather than useful |
| M-UX-DR-08        | 🟢 LOW   | ✅ RESOLVED 2026-04-25 — invariant landed AND caught real drift | `breadcrumb-height-matches-token` test added to functional-tests.json; on first run flagged actual rendered height = 44.91px vs token 52px (T8 sticky-rule relocation shrank reserved space ~7px); token re-measured to 46px, test bound `[44, 48]`. Sticky rails now sit tight to the breadcrumb |
| M-UX-DR-09        | 🟢 LOW   | ✅ RESOLVED 2026-04-25 | 2 mobile test cases added: `drawer-trigger-visible-mobile` (375×812 — `#drawer-trigger` width > 0 + `aria-controls="drawer"`); `rhs-toc-collapsed-mobile-default` (375×812 — `details.toc-mobile-collapse` `open` attribute null/missing) |
| M-UX-DR-10        | 🟢 LOW   | DEFERRED to a future scripts/ ergonomics sweep (= T9-ISS-03) — explicitly logged in T9 issue file | `argparse.BooleanOptionalAction` for the `--no-sandbox` flag is the recommended fix; not blocking |
| M-UX-DR-11        | 🟢 LOW   | ✅ RESOLVED-WITH-RATIONALE 2026-04-25 — option (a) per T5 audit + auditor recommendation | HomeLayout's mode-detection-block duplication is intentional; T5 audit explicitly authorised verbatim preservation. Re-flag for extraction only if a third layout joins (M5 dashboard candidate) |
| M-UX-DR-12        | 🟢 LOW   | ✅ RESOLVED-WITH-RATIONALE 2026-04-25 — option (a) | 4-page scoped-CSS duplication accepted as intentional (each Astro file owns its own `<style>` block per the framework's per-page-scoped pattern). Extraction to shared `home.css` reconsiders only if a 5th landing page joins or if a visual-style sweep needs single-source theming |
| M-UX-DR-13        | 🟢 LOW   | ✅ RESOLVED 2026-04-25 (subsumed by DR-09 fix) | mobile breakpoints (375) now in the assertion suite alongside the screenshot smoke; bullet 9's "tested at 1280, 1024, 768, 375" claim is now machine-checkable for at least one of those mobile widths |
