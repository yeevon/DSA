...
- [x] **Three-column desktop layout** (≥1024px) — left rail, center content, right rail — renders cleanly on every chapter route (lectures, notes, practice). (T1 + T2 + T4 issue files)
...
- [x] **Right-rail in-chapter TOC** — section anchors SSR-rendered from MDX frontmatter (`sections` array), scroll-spy enhancement as a JS island that toggles `data-current` via `IntersectionObserver`. M3's `SectionNav` is refactored into this slot (no two left rails). TOC filters to top-level numbered sections (`^\d+\.\d+\s` — e.g. "1.6 Vector Resize") so subsection anchors stay reachable in-article without cluttering the rail (b29d409, 2026-04-25). (T4 issue file)
...
- [x] **Deploy contract preserved** — site ships **40 prerendered pages** (37 chapter routes + 3 collection-landing pages added in T9); M3 surfaces still gated on `data-interactive-only` and invisible in static mode; no server-only code leaks into the client bundle. `dist/client/` cumulative size delta vs pre-M-UX baseline measured at **+756 KB / +17.5%** post-b29d409 (was +873 KB / +20.2% post-T9, before the RHS-TOC top-level filter shrank the bundle by ~117 KB by dropping subsection TOC entries; was +817 KB / +18.9% post-T8; was +1.42 MB before T7 cycle 2's single-DOM-tree refactor recovered 618 KB; the literal +50KB figure cited in early M-UX scoping was a back-of-envelope guess before the layered chrome scope was understood, identified as somewhat arbitrary by the user). Residual delta dominated by the CompletionIndicator SSR-embed manifest (~432 KB) — see [`nice_to_have.md`](../../nice_to_have.md) §UX-4 for promotion trigger. Sticky breadcrumb scroll regression (M-UX-T3-ISS-01) caught + fixed during T8 deploy verification. T9 added the Selenium functional-test harness (`scripts/functional-tests.py`) — **22 test cases / 34 assertions** post-deep-review covering wide-viewport, sticky-rail, cross-collection, breadcrumb-link, RHS-TOC top-level-filter, breadcrumb-height invariant (caught real drift at deep review — 52px token vs 44.91px actual; token re-measured to 46px), and mobile-viewport (drawer-trigger + RHS TOC collapsed-default at 375×812) contracts at gate-time. `data-interactive-only` carrier count on `dist/client/lectures/ch_4/index.html` = **37** post-b29d409 (was 87 pre-filter; subsection read-indicator slots dropped along with the subsection TOC entries). (T8 issue file + T9 issue file + [`m_ux_deep_review.md`](issues/m_ux_deep_review.md) milestone-level audit + [`design_docs/m_ux_deploy_verification.md`](../../m_ux_deploy_verification.md))
...
| T3  | [Top breadcrumb — collection switcher + prev/next + sticky](tasks/T3_breadcrumb.md) | ✅ done 2026-04-24 |
