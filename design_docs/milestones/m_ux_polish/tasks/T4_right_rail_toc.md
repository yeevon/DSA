...
3. Build `ScrollSpy.astro`:
   ...
   - On mount, query the section anchors inside the article container that match TOC entries. **Selector: `article a[id^="ch_"]`** (resolves MUX-BO-DA2-D / LOW + MUX-BO-DA3-D / LOW) — this matches T1's preserved `<article>` wrapper contract (MUX-BO-ISS-01 / HIGH-1; T1 Step 2 + T1 audit-check #7) **and** mirrors M3 `MarkReadButton.astro` line 70's existing `article a[id^="ch_"]` pattern, so both islands observe the same anchor set. Pinning the selector means a future `<article>`-rename or anchor-prefix change surfaces immediately as a HIGH ScrollSpy regression instead of a silent no-op (an unscoped `[id]` query would still find the headings via document-wide search and mask the breakage; a less specific `article [id]` would also match unrelated `[id]` descendants if the MDX content ever introduces them). Don't write `document.querySelectorAll('[id]')` — write `document.querySelectorAll('article a[id^="ch_"]')`. (DA3-D resolves the prior vocabulary↔selector mismatch — earlier prose said "headings" while the pinned selector was the broader `article [id]`; tightening to `article a[id^="ch_"]` keeps both the wrapper-contract guarantee from DA2-D and the pandoc-emitted-anchor specificity that M3 MarkReadButton already relies on.)
...
- [ ] **Live TOC indicator refresh on `cs300:read-status-changed`** (resolves MUX-BO-DA2-A / MEDIUM + MUX-BO-DA3-C / LOW — exercises the listener requirement pinned in Step 4 per MUX-BO-DA-3). Auditor opens `/DSA/lectures/ch_4/` in `npm run dev`, picks a section whose TOC entry currently shows the un-marked indicator state, marks that section read via the M3 `MarkReadButton`, **does not reload**, observes the corresponding TOC entry's `[data-read-indicator]` span flip from un-marked to marked (`data-read="true"`) within a few seconds (typically sub-second on localhost; the constraint is "before the user reloads", not a hard latency budget — DA3-C softens an earlier "~1s" wording that risked false-failing on slower connections). If the indicator only updates after reload, the `cs300:read-status-changed` listener wasn't wired in Step 4's read-status island — Step 4's listener requirement is contract-only without this AC enforcing it. Auditor cites the manual-test result (DOM-attribute observation in DevTools or the visible indicator change) in the T4 issue file. Cross-reference: T2's `CompletionIndicator` subscribes to the same event (per MUX-BO-DA2-B) — both must update live, and asymmetric behaviour (one updates live, the other doesn't) is a HIGH finding.
