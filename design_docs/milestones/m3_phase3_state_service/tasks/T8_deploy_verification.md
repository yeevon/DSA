...
1. Compare build sizes:
   - Get M2 close baseline: check git log around commit `bdc1bac` (post-T8 / M2 close): page count, `du -sh dist/`. Save the numbers.
...
4. Push the local commits + verify the actual GitHub Actions deploy ships the same `dist/`. Curl the live URL post-deploy: same checks as step 2 (mode = static, no interactive surfaces visible).
