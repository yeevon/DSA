...
- **Changed** **Workflow infra — `.claude/` restoration from generalized template + 13 review fixes**
  ...
  Added 5 generic operator skills (`check`, `dep-audit`, `ship`, `sweep`, `triage`) adapted for cs-300 (no PyPI / wheel; `dist/` secret-scan instead of artifact-contents check; `ship` repurposed as pre-merge-to-main gate). Added `/check-claim` command for architect Trigger B (external-claim review against LBDs).
  Deep-analysis reports under `runs/workflow-review/` (architect: MISALIGNED → fixed; sr-dev: FIX-THEN-SHIP → fixed; sr-sdet: FIX-THEN-SHIP → fixed). 13 findings closed: gate-parse regexes (was blocking every autonomy run because regexes didn't match the actual `[<name>] ... ✓` smoke footers), analysis-artifact path mismatch (was causing /autopilot infinite-loop on cleaned milestones), LBD-15 pre-flight added to `/clean-implement`, schema links added to 3 agents, project-development skill description compressed to 187 chars, `/clean-tasks` `DONE` → `CLEAN/LOW-ONLY` token alignment, `dependency-auditor` section header date, `long_running_pattern.md` shorthand format, `integrity_checks.md` task-kind diff-based heuristic (cs-300 specs don't carry `**Kind:**` field), `ship/runbook.md` uvx replacement, CLAUDE.md issue-file template completeness, `/clean-implement` escalation note. Two MEDIUM findings deferred (status-surface defense-in-depth Check 4, `/autopilot` path layout cosmetic) — neither blocking. Dep audit: skipped — no manifest changes.
