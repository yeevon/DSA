...
- For an exploratory branch that's outside the autonomy-mode flow but needs the same
  lens-set.

## When NOT to use

- During `/auto-implement` runs — the terminal gate already runs the three reviewers;
  `/sweep` duplicates that.
- For post-halt diagnosis — use `/triage` instead.
- For pre-merge-to-main inspection — use the `dep-audit` Skill (or `/ship` for the full
  merge gate).
...
- `--base <ref>` — diff against a specific base ref (e.g. `--base main` for
  branch-vs-main).
...
3. Spawn sr-dev + sr-sdet + security-reviewer in parallel via three `Agent` tool calls
   in one orchestrator turn (per `.claude/commands/_common/parallel_spawn_pattern.md`).
   Each reviewer writes its fragment to `runs/sweep/<timestamp>/<reviewer>-review.md`.
4. After all three complete, parse the 3-line return-schema verdicts.
5. Apply the precedence rule from `.claude/commands/_common/parallel_spawn_pattern.md`
   (see `runbook.md` §Precedence rule):
   - any BLOCK → `SWEEP-BLOCK`;
   - any FIX-THEN-SHIP (no BLOCK) → `SWEEP-FIX`;
   - all SHIP → `SWEEP-CLEAN`.

## Outputs

Write `runs/sweep/<timestamp>/report.md` (consolidated) plus per-reviewer fragments:

- **Consolidated report** — overall verdict + per-reviewer summary line + pointer to
  each fragment.
...
`SWEEP-CLEAN | SWEEP-FIX | SWEEP-BLOCK | NO-DIFF`. `file:` = consolidated report path.
`section:` = `—`.

## Helper files

- `runbook.md` — spawn-prompt templates per reviewer; precedence-rule reminder; example
