...
`$ARGUMENTS` is optional. If empty, scan all open milestones. If a milestone-list shorthand (e.g. "m4 m5") is given, scope the walk to those milestones only.

This command spawns the `roadmap-selector` agent, reads its recommendation, and reports the next action the user should run. **It does not chain into `/auto-implement` or `/clean-tasks` directly** — composite autonomy lives in a separate orchestrator (`/autopilot`) that inlines all three procedures.
...
After the `roadmap-selector` `Task` spawn, parse the agent's return per [`_common/agent_return_schema.md`](_common/agent_return_schema.md):
...
4. The `verdict` value must be one of `PROCEED`, `NEEDS-CLEAN-TASKS`, `HALT-AND-ASK` (roadmap-selector's allowed tokens); trailing whitespace on any value is stripped before validation.
5. On any failure: halt, surface `BLOCKED: agent roadmap-selector returned non-conformant text — see the raw return file`. **Do not auto-retry.**
...
Pass only what the `roadmap-selector` will certainly use. Let the agent pull milestone README contents and task spec details on demand via its own `Read` tool.

After the `Task` spawn, capture the spawn-prompt token count (regex proxy: `len(re.findall(r"\S+", text)) * 1.3`, truncated to int) into `runs/queue-pick-<ts>/spawn_roadmap-selector.tokens.txt`.

### roadmap-selector spawn

Minimal pre-load set: recommendation file path, project context brief, milestone scope (from `$ARGUMENTS`, or "all open").

**Remove from inline content:** full milestone README content, full task spec content, architecture-doc content.
...
- **Empty:** roadmap-selector scans every open milestone directory listed in `design_docs/milestones/README.md`.
- **"m4 m5" / "m4":** pass the milestone list verbatim to roadmap-selector.
...
If the resulting path does not exist on disk, that's a pre-condition failure — the orchestrator was invoked from a directory Claude Code has not yet seen. Halt and surface the question (in normal operation the auto-memory dir is created on first conversation in the project).
...
Recommendation file path: `runs/queue-pick-<YYYYMMDD-HHMMSS>.md`. Create the path; pass to the agent.
...
Next task: <task-spec-path>
   ...
   Run: /auto-implement <milestone-shorthand> <task-shorthand>
        e.g. /auto-implement m4 t01
```

Followed by a one-paragraph summary of the agent's reasoning (sourced from the recommendation file's `## Reasoning` section).

If the agent's `## Decisions the orchestrator should expect during this task` block names substantive arbitration (semver bump, new-decision proposal, deferral), surface those bullets verbatim — the user should know what `/auto-implement` will surface for them.
...
For fully-autonomous queue draining (queue-pick + clean-tasks + auto-implement chained without user input between steps), see `/autopilot` — it inlines all three procedures so a single invocation drains the queue end-to-end.
