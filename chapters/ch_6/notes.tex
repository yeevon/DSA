...
\title{CS 300 -- Chapter 6 Notes\\\large Trees}
\author{Jose de Lima}
\date{\today}
...
\begin{itemize}
    ...
    \item \textbf{Height vs.\ balance}: a balanced tree gives $h = O(\log
          ...
          Motivates AVL / red-black trees (ch.~7 onward).
...
\textbf{Looking ahead.} The whole \emph{week 7+ / ch.~7--9} arc is about
...
\texttt{30\ \ 20} \\
\end{tabular} \\
BST: left $\leq$ root $\leq$ right & Not a BST: \\
 & 30 is in 10's left subtree but $> 10$ \\
\end{tabular}
\end{center}
...
\begin{itemize}
    \item \textbf{Chapters 7-8 (AVL, red-black).} These make the BST
...
\texttt{cheat\_sheets/ch\_6.tex}. Practice prompts (problem $\to$
pseudocode $\to$ C++): \texttt{practice\_prompts/ch\_6.md}.
