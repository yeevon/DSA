...
\title{Ch.~5 Notes\\\large Hash Tables and Cryptographic Hashing}
\author{}
\date{}
...
Defense against adversarial input: pick the hash function randomly
from a family $\mathcal{H}$ where
$\Pr_{h \in \mathcal{H}}[h(k) = h(l)] \leq 1/m$ for all $k \neq l$.
Canonical family: $h_{a,b}(k) = ((ak + b) \bmod p) \bmod m$ with
prime $p > u$ and random $a \in [1, p{-}1], b \in [0, p{-}1]$.
Expected chain length $\leq 1 + \alpha$ \emph{for any keys}. Real
systems (Python, Rust, Java) approximate this with a startup-randomized
seed plus a fast non-cryptographic hash.
