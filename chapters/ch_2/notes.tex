...
\title{CS 300 -- Chapter 2 Notes\\\large Introduction to Algorithms}
\author{Jose de Lima}
\date{\today}
...
\emph{language} for comparing algorithms (big-O, which starts in
\textsection 2.2) so you can justify your choice. When you finish, the
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Test with the tiny example from step 2.} If the
          output does not match, trace the loop (\textsection 1.12
          methodology).
\end{enumerate}
...
A recursive rewrite of the reversal loop from \textsection 1.11.
...
\end{verbatim}
Compare to the loop in \textsection 1.11. The recursion mirrors the
...
linear $O(n)$ scan from \textsection 1.9.
...
\section{2.11 \textit{(next section -- ready to scrape)}}
...
\texttt{cheat\_sheets/ch\_2.tex}. Practice prompts (problem $\to$
pseudocode $\to$ C++ workflow): \texttt{practice\_prompts/ch\_2.md}.
