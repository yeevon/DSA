...
\title{Ch.~2 Notes\\\large Algorithms, Recursion, Strategies}
\author{}
\date{}
...
\subsection*{DP: the template (SRTBOT)}
OCW 6.006 mnemonic: \textbf{S}ubproblem $\to$ \textbf{R}elate $\to$
\textbf{T}opo order $\to$ \textbf{B}ase $\to$ \textbf{O}riginal $\to$
\textbf{T}ime. In practice:
\begin{enumerate}
    ...
    of smaller states. Pattern: \emph{guess + brute-force} over the
    possible answers to a question.
    \item Identify \textbf{base cases}.
    \item Choose \textbf{topo order} to fill the table (bottom-up) or
    use memoization (top-down). Subproblems must form a DAG.
...
\subsection*{``LCS'' is ambiguous}
Most textbooks (CLRS, MIT 6.006) use \textbf{LCS} for longest common
\emph{subsequence} (non-contiguous; gaps allowed). The cs-300 lecture
covers longest common \emph{substring} (contiguous). Different problem,
different recurrence. Match $L[i][j]$ on mismatch:
\begin{tabular}{@{}ll@{}}
substring   & $0$ (reset)\\
subsequence & $\max(L[i{-}1][j],\ L[i][j{-}1])$\\
\end{tabular}
...
\begin{itemize}
    ...
    \item ``DP solves knapsack in $O(nW)$'' is \textbf{pseudopolynomial},
    not strongly polynomial: $W$ can be exponential in input bits.
    Knapsack remains NP-hard.
\end{itemize}
