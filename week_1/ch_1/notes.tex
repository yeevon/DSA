...
\title{CS 300 -- Chapter 1 Notes\\\large Review: Flowcharts, Pseudocode, and Arrays and Vectors}
\author{Jose de Lima}
\date{\today}
...
\subsection{What a program actually is}

A \textbf{program} is a sequence of instructions a machine executes in order
to transform inputs into outputs. The instructions are written in a
\textbf{programming language} -- which can be visual (boxes connected by
arrows in a flowchart), textual (source code like C++ or Python), or
something in between. The source text uses \textbf{Coral} because its
flowchart form makes control flow visible; this course will move to
\textbf{C++} for the actual projects. Everything in this chapter has a
direct C++ counterpart, so focus on the \emph{ideas}, not the Coral syntax.

\begin{defnbox}[Interpreter vs.\ compiler]
Both run a program, but at different moments:
\begin{itemize}
    \item An \textbf{interpreter} reads a statement and executes it
          immediately. Coral's browser environment is an interpreter. So is
          Python's default runtime.
    \item A \textbf{compiler} translates the whole program into machine code
          \emph{before} anything runs; the machine code is what actually
          executes. C++ uses a compiler (e.g., \texttt{g++}, \texttt{clang++}).
\end{itemize}
The practical difference: compiler errors surface before runtime,
interpreters surface them on the line that tries to execute.
\end{defnbox}

\subsection{Statements and execution order}

A program is a list of \textbf{statements}. Each statement carries out a
single unit of work -- read a value, perform a calculation, print output --
and they execute \textbf{one at a time}, top to bottom, until control flow
(branches, loops, function calls) sends the program somewhere else.

In a Coral flowchart each statement lives in its own shape (rectangles for
assignments, parallelograms for I/O, diamonds for branches). The shape tells
you what kind of statement it is; the arrows tell you the order.

\subsection{Variables}

\begin{defnbox}[Variable]
A named slot of memory that holds a value. A variable has three parts you
should always be able to name:
\begin{enumerate}
    \item a \textbf{name} (the identifier you reference it by),
    \item a \textbf{type} (what kind of value it can hold -- integer, string, $\ldots$),
    \item a \textbf{value} (what it currently holds; may change over time).
\end{enumerate}
\end{defnbox}

Mental model: think of a variable as a labeled box. The label is the name
(\texttt{wage}), the box's shape constrains what fits inside it (\emph{integer}
-- whole numbers only), and the contents are the value. Writing \texttt{wage = 20}
puts the number \texttt{20} into the box labeled \texttt{wage}, replacing
whatever was there before.

\subsection{Assignment}

The \textbf{assignment statement} stores a value into a variable:
\begin{lstlisting}
wage = 20
salary = wage * 40 * 50
\end{lstlisting}

Read the equals sign as ``becomes,'' not as ``is equal to.'' Assignment is a
\emph{command}, not a \emph{question}. The right-hand side is evaluated first,
then the resulting value is dropped into the variable on the left.

\begin{warnbox}[\texttt{=} vs.\ \texttt{==}]
In most languages (including C++), \texttt{=} is \textbf{assignment} and
\texttt{==} is \textbf{equality test}. Mixing them is one of the most common
beginner bugs. \texttt{if (x == 5)} asks a question; \texttt{if (x = 5)}
quietly \emph{assigns} \texttt{5} to \texttt{x} and evaluates to true. Coral
sidesteps the issue with explicit wording, but C++ will not warn you.
\end{warnbox}

\begin{examplebox}[Walkthrough: wage / salary]
\begin{enumerate}
    \item Two integer variables exist: \texttt{wage} and \texttt{salary},
          each initialised to \texttt{0}.
    \item Execution starts at the start node. The first statement sets
          \texttt{wage = 20}.
    \item The next statement computes \texttt{wage * 40 * 50} -- that is,
          \texttt{20 * 40 * 50 = 40000} -- and assigns it to \texttt{salary}.
    \item Two output statements then print \texttt{"Salary is "} followed by
          \texttt{salary}'s value (\texttt{40000}).
    \item Execution reaches \texttt{End}.
\end{enumerate}
\end{examplebox}

\subsection{Input}

Programs rarely hard-code their data. They get it from \textbf{standard
input} -- conceptually, a stream of values that flows \emph{into} the program
from the keyboard, a file, a redirected pipe, or a form field.

\begin{defnbox}[Input statement (Coral)]
\texttt{variable = Get next input} assigns \texttt{variable} with the next
value waiting on the input stream. A parallelogram represents this in a
flowchart.
\end{defnbox}

\begin{examplebox}[Coral vs.\ C++ input]
\begin{tabular}{ll}
\textbf{Coral} & \texttt{x = Get next input} \\
\textbf{C++}   & \texttt{std::cin >> x;} \\
\end{tabular}\\[4pt]
Both read one value from the input stream and store it in \texttt{x}. C++
has to know \texttt{x}'s type (declared earlier) so it can parse the input
text into that type (integer, double, string, $\ldots$).
\end{examplebox}

\subsection{Output}

The mirror image of input. Programs write to \textbf{standard output} --
a stream that flows \emph{out} of the program to the terminal (or wherever
the user has redirected it). In Coral, a parallelogram again:
\begin{lstlisting}
Put item to output
\end{lstlisting}
The item is either a \textbf{string literal} (quoted text, like
\texttt{"Hello"}) or a \textbf{variable} (unquoted, like \texttt{salary}).
Quotes are the difference between ``print this exact text'' and ``look up
this variable and print its value.''

\subsubsection{Cursor and newlines}

A \textbf{cursor} tracks where the next character of output will land. Each
output statement prints starting at the cursor's current position and
advances it past the last character printed. There is no automatic line
break -- consecutive output statements stack left-to-right on the same line
unless you tell them otherwise.

\begin{defnbox}[Escape sequence]
Inside a string literal, some characters cannot be typed literally (you
cannot just press Enter to put a newline inside quotes). An \emph{escape
sequence} is a two-character notation that stands in for one control
character. The most common are:
\begin{itemize}
    \item \texttt{\textbackslash n} -- newline (moves cursor to the next line)
    \item \texttt{\textbackslash t} -- tab
    \item \texttt{\textbackslash\textbackslash} -- a literal backslash
    \item \texttt{\textbackslash"} -- a literal double quote
\end{itemize}
The backslash itself never appears in the final output; the escape sequence
is \emph{replaced} by the single character it represents.
\end{defnbox}

\begin{examplebox}[Coral vs.\ C++ output]
\begin{tabular}{ll}
\textbf{Coral} & \texttt{Put "Salary is " to output} \\
               & \texttt{Put salary to output} \\
\textbf{C++}   & \texttt{std::cout << "Salary is " << salary;} \\
\end{tabular}\\[4pt]
C++ chains multiple items with \texttt{<<}. To force a newline in C++ use
\texttt{"\textbackslash n"} or \texttt{std::endl}.
\end{examplebox}

\begin{ideabox}[The big picture for 1.1]
Programs are \emph{instructions acting on variables}, driven by a stream of
input and producing a stream of output. Everything else in this course --
data structures, algorithms, file I/O -- is built on top of those four
primitives: variables, assignment, input, output. Get comfortable tracing
execution by hand (``what is \texttt{wage} here? what's the cursor doing?'')
before moving on; every later bug is easier to spot when you can walk the
program one statement at a time.
\end{ideabox}

\section{1.2 Code and Pseudocode}

\subsection{Why text, not pictures}

Flowcharts are great when you are first learning to read control flow --
branches and loops become visible as shapes and arrows. They scale badly,
though. A real program has dozens to thousands of statements; drawing (and
maintaining) a flowchart for even a moderately-sized algorithm is tedious,
and the resulting picture is hard to share, diff, or search. Text wins on
all three: it is compact, easy to type, and lives happily in a file you can
version-control.

Every mainstream programming language -- C, C++, Java, Python, Go -- is a
\textbf{textual} language. Flowcharts are learning scaffolding; the
professional tool is code.

\subsection{Pseudocode}

\begin{defnbox}[Pseudocode]
Text that resembles a program but is written for humans first, not a
compiler. It uses enough programming-style notation to be unambiguous
about \emph{what happens}, but skips the details that a real language
would demand (types, semicolons, header files, etc.).
\end{defnbox}

Pseudocode sits between plain English (``sort the list'') and real code
(\texttt{std::sort(v.begin(), v.end());}). You use it to \emph{design}
an algorithm before committing to a language. The audience is another
programmer -- or, more often, your future self three days later.

\paragraph{What pseudocode typically keeps.}
\begin{itemize}
    \item One statement per line.
    \item Named variables and obvious assignments (\texttt{x = 0}).
    \item Control flow keywords (\texttt{if}, \texttt{for}, \texttt{while},
          \texttt{return}).
    \item Indentation to show structure.
\end{itemize}

\paragraph{What pseudocode typically drops.}
\begin{itemize}
    \item Type declarations (\texttt{int x}).
    \item Punctuation ceremony (semicolons, braces, header files).
    \item Language-specific library calls -- write
          \texttt{swap(A[i], A[j])} rather than
          \texttt{std::iter\_swap(a+i, a+j)}.
\end{itemize}

\begin{warnbox}[Pseudocode is not a language]
Different textbooks, papers, and instructors use slightly different
conventions. There is no single grammar for pseudocode -- the only rule
is that any competent programmer should be able to read it and translate
it into whatever language they are working in. When in doubt, lean on
clarity, not cleverness.
\end{warnbox}

\subsection{Coral code vs.\ Coral flowcharts}

Coral ships in two forms: a \textbf{flowchart} form (the shapes and
arrows from Section~1.1) and a \textbf{code} form (text). Both describe
the same program -- the Coral simulator can convert between them
automatically. Coral code \emph{looks} like pseudocode but is stricter:
it follows rules that let the simulator actually execute it.

\begin{examplebox}[Same program, three notations]
The ``wage / salary'' program from Section~1.1, written three ways:
...
\begin{lstlisting}
int wage   = 20;
int salary = wage * 40 * 50;
std::cout << "Salary is " << salary;
\end{lstlisting}

The \emph{algorithm} is identical in all three. Only the ceremony
changes: pseudocode drops everything optional, Coral declares types
explicitly, C++ adds punctuation and the \texttt{std::cout} call.
\end{examplebox}

\begin{ideabox}[Why this matters for CS 300]
Most of the ``design'' work in this course happens in pseudocode:
algorithms for hash-table probes, BST insertion, tree traversal, and so
on. You sketch the logic in pseudocode, verify it by hand, \emph{then}
translate to C++. Skipping the pseudocode step is why so many first
implementations compile fine and still do the wrong thing -- you coded
a solution before you had one.
\end{ideabox}

\section{1.3 Arrays and Vectors (general concept)}
...
\section{1.5 Array / Vector Iteration Drill}
...
\section{1.6 Iterating Through Vectors}

Section~1.5 covered the \emph{patterns} (running best, counter, scan-plus-swap).
...
\begin{itemize}
    ...
    \item \textbf{Seeding \texttt{maxVal} or \texttt{minVal} with 0.}
          Covered in 1.5. Seed with \texttt{v.at(0)} instead.
...
Most of the loops in 1.5 and this section have a one-liner equivalent
...
\section{1.9 Vector push\_back, back, and pop\_back}

Section~1.8 already introduced \texttt{push\_back} as a way to grow a
...
\begin{ideabox}[The big picture for 1.9]
...
\section{1.10 Modifying, Copying, and Comparing Vectors}
...
Worth knowing, since 1.8 introduced capacity:
...
\begin{ideabox}[The big picture for 1.10]
...
\section{1.11 Swapping Two Variables}
...
\subsection{Why the na\"ive version fails}
...
\section{1.12 Debugging Example: Reversing a Vector}
...
\textsection 1.11: assignment copies one way only.
\end{examplebox}
...
The final correct version was already given in \textsection 1.11. What
...
\section{1.13 Arrays vs. Vectors}
...
\section{1.14 Two-Dimensional Arrays}
...
\section{1.15 Char Arrays / C Strings}
...
\section{1.16 String Library Functions}
...
\section{1.17 Char Library Functions: \texttt{cctype}}
...
\begin{itemize}
    ...
    pattern from 1.9 \emph{is} the stack interface.
\end{itemize}
\end{notebox}

\textbf{Companion materials.} A one-page cheat sheet for this chapter lives
at \texttt{cheat\_sheets/ch\_1.tex}; twelve drill prompts you can paste into
a fresh Claude session live at \texttt{practice\_prompts/ch\_1.md}. Use the
cheat sheet to review, the prompts to test.
